...
void show(int n, vector<vector<int>> &visited) {
...
void maze (int i, int j, int n, vector<vector<int>> &arr, vector<vector<int>> &visited) {
    if( arr[n-1][n-1]==0) {         //if end is blocked
        return;
    }
    ...
    if( isItSafe(i+1, j, n) and arr[i+1][j]==1 and visited[i+1][j]==0 ) {
    ...
    //     maze(i-1, j, n, arr, visited);
    // }
    if( isItSafe(i, j+1, n) and arr[i][j+1]==1 and visited[i][j+1]==0 ) {
